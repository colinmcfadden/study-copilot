...
function App() {
        ...
        {subItems.map((subItem, index) => (
          <div key={index} className="flex flex-col">
            <div className="py-1 px-2 flex justify-between items-center border-b-3 border-b-gray-100">
              <div><a className="underline cursor-pointer" onClick={handleShowNotes}>{subItem.value}</a></div>
  ...
  return (
    <>
      ...
      <div className="card">
        <div className="input-container flex items-center">
          <input type="text"
            ...
            className="field-sizing-content min-w-48 max-w-120 rounded-full border border-slate-300/70 px-5 py-4 !outline-none 
              transition-all duration-200 ease-out placeholder:tracking-widest focus:border-primary focus:ring-transparent"
          />
          ...
          {subItems.map((item, index) => (
            <div key={index}>
              <div className="p-1.5 m-0 flex justify-between items-center border-b-3 border-b-gray-100">
                <div><a className="underline cursor-pointer" onClick={handleShowNotes}>{item.value}</a></div>
                <div>
                  <button className='p-2 rounded-full cursor-pointer bg-mint-400 transition duration-150 ease-in-out hover:bg-mint-500'
      ...
      {showNotes ?
        <div className="shadow-lg border-1 border-gray-200 mt-5 absolute top-40 pt-5 bg-neutral-100 p-4 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-3xl">
          <h2>Notes</h2>
